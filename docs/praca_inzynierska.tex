...
\par Dane testowe są plikami tekstowymi, w których zawarte są ogólne informacje takie jak: liczba kursów, liczba dostępnych pomieszczeń, liczba dni na które mają być rozłożone zajęcia, liczba programów nauczania oraz ograniczenia precyzujące w jakich terminach nie mogą odbywać się zajęcia z danego kursu. Kolejno sprecyzowane są kursy, pomieszczenia, programy nauczania oraz ograniczenia, w poniższym formacie:
\begin{verbatim}
...
\item Na program nauczania składa się określona liczba kursów, których identyfikatory zawarte są w opisie programu nauczania $<Ids\ Kursy>$.
\item W ramach ograniczeń umieszczona jest informacja o kursie $<Id\ Kurs>$, który nie może odbyć się w danym dniu w danym okresie.
\end{enumerate}
...
Tabu tenure $t(c_{i})$ obliczane jest na podstawie obecnie uzyskanego rozwiązania oraz częstotliwości przenoszenia zajęć z kursy $c_{i}$ oznaczonej przez $freq(c_{i})$.
Tabu tenure wyrażone jest poniższym wzorem: \\ 
$t(c_{i}) = funkcja\ kary\ dla\ obecnego\ rozwiązania + \alpha * freq(c_{i})$ \\ \\
